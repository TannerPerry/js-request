...
button.addEventListener('mouse', buttonStyleOff)
// PROBLEM 4
/*
...
.then((res) => {
    ...
    const anim = document.createElement("p")
    newBody.appendChild(anim)
    anim.textContent = newArr[i]
...
const queryTest = () => {
    ...
    axios.get(`http://localhost:3000/query-test?myquery=tanner-query`)
    .then((res) => {
        let repeatText = document.getElementById('repeat-text');
        repeatText.style.display = 'block';
...
//     foodInput.textContent = response.data
//     foodInput.style.display ="block"
// })
// }
// document.getElementById('enter-food-button').addEventListener('click', createFood)
...
enterFoodButton = document.getElementById("enter-food-button")
enterFoodButton.addEventListener("submit", createFood)
